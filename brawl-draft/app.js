...
function renderTeamCounter() {
  ...
  for (const b of BRAWLERS) {
    if (used.has(b.name)) continue;
    const ctx = contextualWinRate(b.name);
    const per = enemyNames.map((en) => {
      const p = predictedWinRate(b.name, en);
      const oa = BRAWLER_OVERALL[b.name];
      const ob = BRAWLER_OVERALL[en];
      const expected = oa !== undefined && ob !== undefined ? 50 + (oa - ob) : 50;
      return { enemy: en, wr: p.wr, edge: p.wr - expected, measured: p.measured };
    });
    const risk = threats.length ? residualRisk(b.name, threats) : null;

    // Il caso peggiore va misurato in percentuale di vittorie NEL CONTESTO,
    // non come matchup nudo. Senza questo, un brawler di cui non sappiamo
    // nulla avrebbe tutte le previsioni vicine al 50% e sembrerebbe il più
    // solido di tutti: l'ignoranza verrebbe premiata. Partendo dalla win
    // rate su questa mappa e sottraendo il matchup peggiore, un brawler
    // debole resta debole anche se nessuno lo punisce in particolare.
    const worstEdge = Math.min(
      per.length ? Math.min(...per.map((p) => p.edge)) : 0,
      risk ? risk.edge : 0
    );
    const floor = Math.max(5, Math.min(95, ctx.base + worstEdge));
    rows.push({ name: b.name, per, risk, floor, base: ctx.base });
  }
  const scored = rows.sort((a, b) => b.floor - a.floor);

  if (enemyNames.length > 0) {
    html += `<p class="panel-title spaced">I più solidi contro chi è già in campo</p>`;
    for (const r of scored.slice(0, 5)) {
      const chips = r.per
        .map((p) => `<span class="echip ${p.measured ? "meas" : "est"} ${p.wr >= 50 ? "pos" : "neg"}" title="${p.measured ? "matchup misurato" : "previsione dal modello"}">vs ${p.enemy} ${fmt(p.wr)}</span>`)
        .join("");
      const riskChip = r.risk
        ? `<span class="risk-chip ${r.risk.wr >= 45 ? "ok" : "warn"}" title="la peggiore fra le minacce che possono ancora prendere">rischio: ${r.risk.enemy} ${fmt(r.risk.wr)}</span>`
        : "";
      html += `<div class="team-row"><span class="tr-name">${r.name}</span><span class="tr-chips">${chips}${riskChip}</span><span class="tr-total">${fmt(r.floor)}</span></div>`;
    }
    html += `<p class="hint foot">La percentuale a destra è <strong>il tuo caso peggiore</strong>: quanto vinci contro l'avversario che ti va peggio, fra quelli in campo e quelli che possono ancora arrivare. Più è alta, meno rischi.</p>`;
  } else if (threats.length) {
    html += `<p class="panel-title spaced">Pick difficili da punire</p>`;
    for (const r of scored.slice(0, 5)) {
      const riskChip = r.risk
        ? `<span class="risk-chip ${r.risk.wr >= 45 ? "ok" : "warn"}">peggio: ${r.risk.enemy} ${fmt(r.risk.wr)}</span>`
        : "";
      html += `<div class="team-row"><span class="tr-name">${r.name}</span><span class="tr-chips">${riskChip}</span><span class="tr-total">${fmt(r.floor)}</span></div>`;
    }
    html += `<p class="hint foot">Scegliendo per primo la domanda non è "chi batte chi", ma "cosa non mi possono punire". Questi sono i pick con il caso peggiore più alto contro le risposte più probabili.</p>`;
  }
...
function renderSuggestions() {
  ...
  suggestions.forEach((s, i) => {
    ...
    row.innerHTML = `
      <span class="sugg-name">${s.name}${badge}${chips ? `<span class="chip-row">${chips}</span>` : ""}</span>
