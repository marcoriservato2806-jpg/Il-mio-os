...
function edgeCasellaVuota(candidato, minacce) {
  ...
  //
  // IL CASO PEGGIORE VA MISURATO CON LA STESSA UNITA' DEL RESTO. Qui c'era un
  // residuo della versione senza matrice: la percentuale veniva dal dato vero
  // ma l'atteso da cui la si sottraeva veniva da BRAWLER_OVERALL, che sta su
  // un'altra scala (e' la win rate di CHI GIOCA quel brawler). Due scale dentro
  // la stessa sottrazione.
  //
  // Non era solo una taratura sbagliata: sceglieva il nemico SBAGLIATO. Su
  // Undermine il pavimento di Surge diceva «minaccia Clancy, vantaggio +0,7»
  // quando Clancy contro Surge vale -5,4 — sei punti di ottimismo sul pick
  // sbagliato. E su Bolt diceva 40% invece di 44%: quattro punti di pessimismo.
  // Sbagliava nei due versi, quindi non si vedeva nemmeno come una distorsione
  // costante.
  //
  // Ora il vantaggio e' lo STESSO numero che il riquadrino mostra per gli
  // avversari in campo: vantaggio della coppia piu' quanto quel nemico e' piu'
  // forte del tipico di questa mappa.
  let peggio = media;
  let quale = null;
  for (const t of minacce) {
    if (t === candidato) continue;
    const e = edgeCentrato(candidato, t) + scartoForzaAvversario(t);
    if (quale === null || e < quale.edge) {
      const p = predictedWinRate(candidato, t);
      quale = { enemy: t, wr: p.wr, edge: e, measured: p.measured };
    }
...
function residualRisk(candidateName, threats) {
  ...
  for (const t of threats) {
    ...
    //
    // E il vantaggio deve essere lo STESSO di quello mostrato a schermo per gli
    // avversari in campo — coppia piu' scarto di forza. Qui c'era solo la
    // coppia, mentre `edgeCasellaVuota` ne usava un terzo tipo: tre unita'
    // diverse che finivano tutte nello stesso `Math.min`, quindi il pavimento
    // dipendeva da quale delle tre capitava di guardare.
    const edge = edgeCentrato(candidateName, t) + scartoForzaAvversario(t);
...
// sceglie per primo non puo' essere counterato in anticipo ma verra' counterato
// dopo, quindi vuole un brawler solido senza counter secchi; chi sceglie per
// ULTIMO e' l'unico che puo' prendere uno specialista, perche' dopo di lui non
// sceglie nessuno. E' l'informazione che il numero da solo non da', ed e' quella
// che cambia il TIPO di pick che cerchi.
//
// Sta sopra la lista, non nel riquadro della situazione: sul telefono quel
// riquadro finisce sotto il roster, cioe' fuori schermo proprio al primo pick,
// che e' il turno in cui questo consiglio serve di piu'. Visto nello screenshot
// dell'utente, non dedotto.
function rigaSedia(turn) {
  const own = turn.team;
  const enemy = own === "A" ? "B" : "A";
  const enemyNames = state.picks[enemy].filter(Boolean);
  const enemyLeft = state.sequence.filter((sq) => sq.phase === "pick" && sq.team === enemy && !nomeDelTurno(sq)).length;
  ...
  if (enemyLeft === 0) {
    return "<strong>Ultimo pick.</strong> È l'unica sedia dove uno specialista è sicuro: dopo di te non sceglie nessuno, quindi nessuno può counterare il tuo counter. Prendi la risposta secca, non il pick prudente.";
  }
  if (enemyNames.length === 0) {
    return "<strong>Primo pick, alla cieca.</strong> Non c'è niente da counterare e loro ti vedranno: serve un brawler solido sulla mappa e senza counter secchi, non una risposta a qualcosa che non c'è. Il riquadrino «peggio» è la parte che conta in questa sedia.";
  }
  if (mieiRimasti > 1) {
    return `<strong>Hai due pick di fila e loro ${enemyLeft === 1 ? "uno solo dopo" : "ancora " + enemyLeft}.</strong> Riempi i buchi adesso — prima linea, distanza, quello che serve alla modalità — perché il prossimo turno è ancora tuo e poi non scegli più.`;
  }
  return `<strong>Loro hanno ancora ${enemyLeft === 1 ? "un pick" : enemyLeft + " pick"} dopo di te.</strong> Quello che prendi ora può essere punito: pesa il riquadrino «peggio» quanto il numero grande.`;
}
