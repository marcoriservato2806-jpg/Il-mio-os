...
|---|---|---|
| **Il termine «quanto e' forte l'avversario» usava la win rate di CHI GIOCA quel brawler** | «al primo pick non c'è nessuno buono contro Amber». I contro-Amber c'erano (Berry +9,8, Shelly +8,2, Leon +8,1): il bug era altrove. `BRAWLER_OVERALL` dava Amber 64,9 contro una media di mappa di 50,3 — cioè −14,6 punti — mentre la sua win rate vera su Pinball Dreams è 48,8. Gonfiata dalla rarità, la stessa distorsione che l'app corregge per i dati di mappa e qui no | forza ricavata **dalla matrice stessa** risolvendo `wr = 50 + (forza a − forza b) + adv` (residuo sd 0,41–0,48 punti), verificata contro la win rate di modalità: r = 0,991, pendenza 0,98. **Il termine era uguale per tutti i candidati, quindi non cambiava l'ordine: schiacciava tutta la lista di 5 punti.** Un errore che non cambia l'ordine è il più difficile da vedere, e il controllo giusto è un invariante, non una rilettura |
| **Il pannello counter leggeva la classifica prima che fosse calcolata** | i punteggi accanto alle risposte erano quelli del turno precedente | `renderTeamCounter()` spostato dopo `renderSuggestions()`. Lo stesso errore era già stato fatto con la griglia, ed era già annotato nel codice: **l'ordine delle chiamate di render non è libero se una legge quello che un'altra calcola** |
| **Ho scritto in memoria un tetto che era mio, e ha fermato la ricerca per settimane** | «nessuna fonte pubblica la matrice completa dei matchup, il tetto è del dato non della ricerca». L'utente ha insistito: «questi dati ci sono, altre applicazioni lo fanno». Aveva ragione | La matrice 108×108 per modalità, **più le sinergie**, sta sul bucket pubblico da cui il Draft Helper di brawlplanet legge nel browser. Le pagine mostrano 5+5, il browser scarica tutto. **Una pagina che mostra dieci righe non prova che il dato sia dieci righe: se è interattiva, guarda cosa scarica.** Copertura dal 5,4% all'89-96% |
| **Il «favore per brawler» era in gran parte un artefatto del campione** | stimato dalle 624 coppie pubblicate, che sono per costruzione gli ESTREMI di ogni brawler. Nella matrice vera la media di riga ha deviazione standard 0,26 punti in Gem Grab | sulle coppie vere non si sottrae più niente: `adv` è già il solo effetto della coppia. Aveva anche una validazione split-half r=0,91 — corretta *dentro* il campione a code, ed è per questo che non l'ha smascherata |
| **Il mio script di controllo dava un falso allarme e me lo sono quasi bevuto** | «il migliore di mappa finisce in posizione 7 su TUTTE le mappe». Era il fallback del mio script: il migliore era fra i sei ban del test, quindi assente dalla lista, e la posizione mancante diventava lunghezza+1 = 7 | escludere i bannati dal confronto. **Un numero identico su tutte le mappe è quasi sempre un bug del misuratore, non un difetto misurato** |
...
| **Il record personale era aggregato al livello sbagliato** | Bolt: 5 vittorie su 5 su Deathcap Trap, 1 su 5 su Undermine, e l'app mostrava la media «tuo 80%». Fuori campione il record per BRAWLER fa AUC 0,474, sotto il caso; per mappa 0,629; a tre livelli 0,674 | Modello a tre livelli (brawler + mappa + resto della coppia), ognuno ristretto sul proprio n. **Prima di fidarti di una media, controlla se la varianza sta dentro i gruppi che stai schiacciando.** |
| **Un tetto messo sul totale invece che sulla parte che differenzia** | il termine di mappa e' uguale per tutti i candidati; capparlo insieme al resto gli faceva mangiare tutto il budget e su una mappa andata male l'app smetteva di distinguere fra i pick | Il tetto limita solo brawler + coppia. **Un vincolo va messo sulla quantita' che vuoi limitare, non sulla somma in cui e' annegata.** |
| **Tratti mappa spuntati a mano che restavano attivi sopra il dato misurato** | il pannello si nasconde quando scegli una mappa, ma `state.mapTraits` conservava le spunte e `scoreCandidate` le sommava lo stesso | I tratti valgono solo senza dati di mappa. E che sarebbe doppio conteggio e' MISURATO: la geometria e' gia' dentro le win rate di mappa (Piper -0,46, Wendy -0,49, Nita +0,44, Ash +0,31, tolto l'effetto modalita' che da solo spiega l'84%). |
| **Il punteggio ignorava la forza dell'avversario in campo** | Ash contro Wendy vince il 43,7% e l'app gli dava +6,5, concludendo che affrontare Wendy lo MIGLIORA. Il residuo `misurato - atteso` toglie la differenza di forza, che era l'informazione da tenere | Aggiunto `forzaTipicaMappa - forza(avversario)`: -17,3 contro Wendy, totale -10,8. **Normalizzare per la forza serve a stimare l'effetto della COPPIA; quando la domanda e' "come me la cavo contro QUESTA squadra", la forza va rimessa.** |
| **Un buco nell'array entrava nel calcolo come avversario fantasma** | nella schermata dell'utente: `vs null 51% +0`. `usedNames()` filtrava i null, `state.picks[enemy]` no | Filtrati. **Se una struttura puo' avere buchi, filtrali in OGNI punto che la consuma, non solo nel primo che ti e' venuto in mente.** |
