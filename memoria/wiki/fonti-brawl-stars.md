...
| **storage.googleapis.com/brawlanalyzer-public** | **il bucket pubblico di brawlplanet: usa questo, non le pagine.** `pl-results.json.gz` ha tutte le mappe ranked in un file (win rate, pick rate, star rate, campione, quali sono attive) e `draft/pairs-<modalità>.json.gz` la matrice. Niente HTML, niente schede da scegliere. `script/fetch-mappe-ranked.js` |
| **api.brawlstars.com** (ufficiale Supercell) | `/players/{tag}/battlelog`: le ultime ~25 partite **con entrambe le squadre e il brawler di ognuno**. È l'unica fonte che dà i pick avversari, quindi l'unica che permette di validare l'app sugli esiti veri. Chiave gratis su developer.brawlstars.com, **legata a un IP fisso**. `script/raccogli-partite.js` |
| ~~**brawlplanet.com** (le pagine)~~ | superato dal bucket. Le pagine hanno due schede e quella aperta di default è quella sbagliata: trappola già costata un giro | **Fonte principale per mappe e modalità dal 6/9.** `/maps/<nomesenzaspazi>_<modalitasenzaspazi>` (es. `kaboomcanyon_heist`), `/powerleague` per il pool. Aggiornamento orario. **Attenzione: la pagina ha due schede e quella aperta di default è "Trophy ladder", non "Ranked".** Vedi sotto. |
